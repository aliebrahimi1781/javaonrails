...
public class IPSeeker {
	 ...
	 */
	public static final String IPDAT_NAME="me.jor.ipseeker.ipdat.name";
	// 用来做为cache，查询一个ip时首先查看cache，以减少不必要的重复查找
	private static final Cache<String,IPLocation> ipCache=Cache.getCache(IPSeeker.class.getName());
	 ...
	 * @throws IOException 
	 */
	public String getCountry(byte[] ip) throws IOException {
		// 保存ip，转换ip字节数组为字符串形式
	 ...
	 * @throws IOException 
	 */
	public IPLocation getIPLocation(String ip) throws IOException{
		IPLocation ipLoc = ipCache.get(ip);
		if(ipLoc==null){
			ipLoc=ipCache.putIfAbsent(ip, getIPLocation(Help.getIpByteArrayFromString(ip)));
		}
		return ipLoc;
	}
	/**
	 ...
	 * @throws IOException 
	 */
	public String getArea(byte[] ip) throws IOException {
	 ...
	 * @throws IOException 
	 */
	public String getArea(String ip) throws IOException {
	    return getIPLocation(ip).getArea();
	}
	
	public String getProvince(String ip) throws IOException{
		return getIPLocation(ip).getProvince();
	}
	
	public String getProvince(byte[] ip) throws IOException{
	...
	public String getCity(byte[] ip) throws IOException{
