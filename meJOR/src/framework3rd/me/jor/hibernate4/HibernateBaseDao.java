...
public class HibernateBaseDao implements InterfaceHibernateDao {
	...
	public <T> List<T> findByParameters(final String hql,final Object params) {
		return setParameter(currentSession().createQuery(hql),params).list();
	}
	/**
	 * 返回符合条件的全集
	 * @param hql    待执行的具名参数化hql
	...
	public <T> List<T> findByParameters(final String hql,final String name,final Object params) {
		return setParameter(currentSession().createQuery(hql),name,params).list();
	}
	/**
	 * 返回符合条件的全集
	 * @param hql    待执行的具名参数化hql
	...
	public <T> T uniqResult(final String hql) {
		return (T) currentSession().createQuery(hql).uniqueResult();
	}
	...
	public <T> T uniqResult(final String hql, final Object params) {
		return (T)setParameter(currentSession().createQuery(hql),params).uniqueResult();
	}
	...
	public <T> T uniqResult(final String hql, final String name, final Object value) {
		return (T)setParameter(currentSession().createQuery(hql),name,value).uniqueResult();
	}
	...
	public <T> T findFirstResult(final String hql){
		return (T)getFirstResult(currentSession().createQuery(hql));
	}
	...
	public <T> T findFirstResult(final String hql, final Object... params){
		return (T)getFirstResult(setParameter(currentSession().createQuery(hql),params));
	}
	/**
	 * 返回符合条件的第一条结果
	 * @param hql
	...
	public <T> T findFirstResult(final String hql, final String name, final Object value) {
		return (T)getFirstResult(setParameter(currentSession().createQuery(hql),name,value));
	}
	/**
	 * 返回符合条件的第一条结果
	 * @param hql
	...
	public <E> E get(Class<E> cls,Serializable id){
		return (E)(this.currentSession().get(cls, id));
	}
	/**
	...
	private Set<String> populateIgnoredFieldsSet(String... ignoredFields){
		Set<String> set=new HashSet<String>();
		if(Help.isNotEmpty(ignoredFields)){
			for(int i=0,l=ignoredFields.length;i<l;i++){
				set.add(ignoredFields[i]);
			}
		}
		return set;
	}
	...
	@SuppressWarnings({ "rawtypes" })
	@Override
	public <T> T updateById(T entity, String idname, UpdateFieldTag tag, String... ignoredFields) {
		Set<String> ignored=populateIgnoredFieldsSet(ignoredFields);
		try {
			Session session=currentSession();
			Class cls = entity.getClass();
			if (Help.isEmpty(idname)) {
	...
	@SuppressWarnings("rawtypes")
	@Override
	public int updateByIds(Object entity, Serializable[] ids, String idname, UpdateFieldTag tag, String... ignoredFields) {
		Set<String> ignored=populateIgnoredFieldsSet(ignoredFields);
		try {
			Class cls = entity.getClass();
			Method[] ms = cls.getMethods();
			...
			for (int i = 0; i < ms.length; i++) {
				...
				if (!ignored.contains(fn) && 
					mn.startsWith("get") && !mn.equals("getClass") && 
					!mn.equals("get" + idname.substring(0, 1).toUpperCase() + idname.substring(1))) {
					Object v = m.invoke(entity);
	...
	public int bulkUpdate(final String hql, final String name, final Object value){
		return setParameter(currentSession().createQuery(hql),name,value).executeUpdate();
	}
	/**
	 * 可执行delete或update hql。
	 * @param hql    待执行的具名参数化hql
	...
	public int bulkUpdate(final String hql, final Map<String, Object> params) {
		return (Integer) setParameter(currentSession().createQuery(hql),params).executeUpdate();
	}
	...
	public int bulkUpdate(String hql, Object value) {
		return this.setParameter(currentSession().createQuery(hql), value).executeUpdate();
	}
	@Override
	public int bulkUpdate(String hql, Object... values) {
		return this.setParameter(currentSession().createQuery(hql), values).executeUpdate();
	}
	@Override
	public void flush() {
